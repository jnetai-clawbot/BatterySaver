...
class SettingsFragment : Fragment() {
    companion object {
        private const val TAG = "BS_SettingsFragment"
    }
    ...
    private fun loadSettings() {
        try {
            ...
            DebugLogger.logDebug("Settings loaded: batteryThreshold=$batteryThreshold%, overheatThreshold=${overheatThreshold}°C")
        } catch (e: Exception) {
    ...
    private fun saveIntSetting(key: String, value: Int) {
        try {
            ...
            context.getSharedPreferences("battery_saver_prefs", Context.MODE_PRIVATE)
                ...
                .putInt(key, value)
                .apply()
            DebugLogger.logDebug("Saved setting: $key = $value")
        } catch (e: Exception) {
            DebugLogger.logError(
